...
from det_iden_track.core.factory.registry import Registry
...
class Factory(Registry):
	...
	def build(self, name: str, *args, **kwargs) -> object:
		...
		if name not in self.registry:
			logger.error(f"{name} does not exist in the registry.")
			return None
	...
	def build_from_dict(
		...
		if not isinstance(cfg, (dict, Munch)):
			logger.error("`cfg` must be a dict.")
			return None
		
		if "name" not in cfg:
			logger.error("`cfg` dict must contain the key `name`.")
			return None
	...
	def build_from_dictlist(
		...
		if (not is_list_of(cfgs, expected_type=dict)
			...
			logger.error("`cfgs` must be a list of dict.")
			return None
