...
from det_iden_track.core.type.collection import is_list_of
from det_iden_track.core.type.type import Size2T
...
def to_tensor(
	...
	# NOTE: Normalize
	if normalize:
		from det_iden_track.core.enhance import normalize_naive
...
def to_image(
	...
	if not torch.is_tensor(tensor):
		logger.error(f"Input type is not a Tensor. Got: {type(tensor)}.")
		return tensor
